                ...
                if(json.list.length <= 0){
                    ...
                    $.each(json.list, function (i, v) {
                        ...
                        var html =
                            ...
                            '</td>'+
                            '<td><p class="catalog-title">'+ v.catalog_title+'</p></td>'+
                            '<td><p class="catalog-desc">'+ v.catalog_desc+'</p></td>'+
                            '<td><img width="40px" height="60px" src="'+v.catalog_cover_url+'?x-oss-process=image/resize,w_40,limit_1"></td>'+
                            '<td>'+ auditTxt+'</td>'+
                            '<td>'+ v.publish_time+'</td>'+
                            '<td>'+
                            '<a onclick="show_detail_box('+v.catalog_id+');">查看</a>'+
                            '</td>'+
                            '</tr>';
...
function show_detail_box(id) {
    $('.catalog-detail-box').fadeIn(function(){
        ...
        get_catalog_detail(id,fill_data);
    });
}

function set_list_paginator(){
    if($pageInfo.total > 0) {
